...
init.sim.par.id = function(a = c(1/2, -1/2), params) {
  # Inputs:
  ...
  #   - mom_i : ID of mother (set to 0 for first population)
  #   - dad_i : ID of father (set to 0 for first population)
  init.popn = matrix(sample(a, prob = c(p.pos, 1 - p.pos),
    ...
    mutate(g_i = apply(., 1, sum) / sqrt(n.loci),
           ...
           mom_i = 0,
           dad_i = 0) %>%
    select(i, g_i, z_i, w_i, r_i, fem, gen, mom_i, dad_i, all_of(names.array))
...
#                   sig.e = 0.5, pos.p = 0.5)
# 
# popn0 = init.sim.par.id(params = pars)
# popn0
# 
# pars = data.frame(n.loci = 20, n.pop0 = 40,
#                   w.max = 1.2, theta = 2.6,
...
propagate.sim.par.id = function(a = c(1/2, -1/2), params, popn) {
  ...
  if (any(popn$fem) & any(!popn$fem) & sum(popn$r_i[popn$fem])) {
    ...
    offspring = cbind(
      ...
      popn %>% 
        filter(fem) %>% 
        select(-c(g_i, w_i, z_i, fem, gen, mom_i, dad_i)) %>%
      ...
      gather(key = alls, value = val, -c(i, mom_i, dad_i)) %>%
      ...
      select(-i) %>%
      mutate(g_i = rowSums(.[names.array]) / sqrt(n.loci),
             i = max(popn$i) + 1:nrow(.),
...
#### Testing:
# # Try it out.
# propagate.sim.par.id(a = c(-1/2, 1/2), params = pars, popn = popn0)
# appears to work


# Good.

sim.par.id = function(a = c(1/2, -1/2), params, init.popn = NULL, evolve = TRUE) {
  ...
  all.pop = data.frame(i = rep(NA, init.row),
                       ...
                       fem = rep(NA, init.row),
                       gen = rep(NA, init.row),
                       mom_i = rep(NA, init.row),
                       dad_i = rep(NA, init.row)) %>%
    cbind(matrix(NA, 
                 nrow = init.row,
                 ncol = 2 * n.loci) %>%
            data.frame() %>%
