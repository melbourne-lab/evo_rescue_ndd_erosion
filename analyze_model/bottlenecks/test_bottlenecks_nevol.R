# Simulating non-evolving trials (with and without density dependence)
# SN - adopted from the old bottleneck server script
# "written" and run on server july 1 2020
...
for (i in 1:nrow(pars)) {
  liszt.vhi[[i]] = sim( params = pars[i,], evolve = FALSE)
  print(i)
}
...
write.csv(all.n, na = '', row.names = FALSE,
          file = 'out/bottleneck_nevolve_test_pop_sizes.csv')

# Save all simulation output
save(liszt.vhi, liszt.vlo, file = 'out/bottleneck_nevolve_adapted_outputs.RData')
